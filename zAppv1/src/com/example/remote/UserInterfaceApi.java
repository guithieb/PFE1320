...
public class UserInterfaceApi extends BaseApi {
	...
	public static void sendVolume(String baseUrl, String volumeToSend) throws ServerException{
		Log.d(LOG_TAG, "sendVolume() url=" + (baseUrl + URL_SUFFIX_SEND_VOLUME) + ", volumeToSend=" + volumeToSend);
		try{
			SendVolume sendVolume = new SendVolume(volumeToSend);

			HttpResponse responsevolume = executeHttpPost(baseUrl+ URL_SUFFIX_SEND_VOLUME, sendVolume);

			checkResponseCodeWithoutBody(volumeToSend, responsevolume);
		} catch (ServerException ex) {
			throw ex;
		}  catch(Exception ex) {
			Log.e(LOG_TAG, "Failed to send key due to: " + ex);
